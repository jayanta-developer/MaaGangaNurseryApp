    ...
    <main className={styles.page}>
      ...
      <div className={styles.productLayout}>
        ...
        <section className={styles.summary}>
          ...
          <button className={styles.buyButton} onClick={() => { setIsOrderSubmitted(false); setIsOrderOpen(true); }}>
...
function OrderModal({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response) {
      setErrorMessage("We could not connect to the order service. Please try again.");
      return;
    }

    const result = await response.json().catch(() => null) as { success?: boolean; message?: string } | null;
    if (!response.ok || !result?.success) {
      setErrorMessage(result?.message ?? "We could not submit your order. Please try again.");
      return;
    }
  ...
  return (
    <div className={styles.modalBackdrop} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className={styles.orderModal} role="dialog" aria-modal="true" aria-labelledby="order-title">
        <button className={styles.closeButton} onClick={onClose} aria-label="Close order form">×</button>
        {submitted ? (
          <div className={styles.successState}>
            <span className={styles.successIcon} aria-hidden="true">✓</span>
            <p className={styles.eyebrow}>Order received</p>
            <h2 id="order-title">Thank you for<br /><em>your order.</em></h2>
            <p><strong>Order request submitted successfully.</strong> We have received your request for <strong>{product.name}</strong>. Our team will connect with you shortly to verify your order and confirm the delivery details.</p>
            <button className={styles.confirmButton} onClick={onClose}>Continue browsing</button>
          </div>
        ) : (
          <>
            <p className={styles.eyebrow}>Manual order request</p>
            <h2 id="order-title">Tell us where to<br /><em>send your plant.</em></h2>
            <div className={styles.orderProduct}><span style={{ backgroundImage: `url("${product.image}")` }} /><div><strong>{product.name}</strong><small>{product.price} · {product.category}</small></div></div>
            <form className={styles.orderForm} onSubmit={handleSubmit}>
              <label>Customer name<input name="name" type="text" placeholder="Your full name" required /></label>
              <label>Phone number<input name="phone" type="tel" placeholder="10-digit mobile number" pattern="[0-9]{10}" required /></label>
              <label>Delivery address<textarea name="address" placeholder="House / street / village / city" rows={3} required /></label>
              <label>PIN code<input name="pinCode" type="text" inputMode="numeric" placeholder="6-digit PIN code" pattern="[0-9]{6}" maxLength={6} required /></label>
              <div className={styles.quantityField}>
                <span>Quantity</span>
                <div className={styles.quantityControl} aria-label="Product quantity">
                  <button type="button" onClick={() => setQuantity(Math.max(1, quantity - 1))} aria-label="Decrease quantity">−</button>
                  <strong>{quantity}</strong>
                  <button type="button" onClick={() => setQuantity(Math.min(20, quantity + 1))} aria-label="Increase quantity">+</button>
                </div>
              </div>
              ...
              <button className={styles.confirmButton} type="submit" disabled={isSubmitting}>
                {isSubmitting ? "Submitting request..." : <>Submit order request <span aria-hidden="true">→</span></>}
              </button>
            </form>
            <p className={styles.formNote}>No online payment is needed. We will call you to confirm the order.</p>
          </>
        )}
      </section>
    </div>
